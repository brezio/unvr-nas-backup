...
TZ="${TZ:-UTC}"
EXPORTER_PORT="${EXPORTER_PORT:-8550}"
...
# ── Ensure shared volume state files exist ──────────────────────────────────
mkdir -p /shared
touch /shared/backup.lock

# ── Export env for cron ──────────────────────────────────────────────────────
# Cron jobs don't inherit the container's environment, so we dump it to a file
# that backup.sh will source.
env | grep -E '^(PROTECT_|BACKUP_|BATCH_|ARCHIVE_|SSH_|CRON_|RUN_ON_START|LOG_LEVEL|TZ|PATH=|RETENTION_|S3_|AWS_|API_|EXPORTER_|REMUX_)' \
...
# ── Start trigger server (background) ───────────────────────────────────────
log "Starting trigger server on port ${EXPORTER_PORT}"
python3 /usr/local/bin/trigger.py &
